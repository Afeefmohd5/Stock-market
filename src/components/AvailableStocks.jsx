...
function AvailableStocks({ onInvest, darkMode }) {
  ...
  const handleInvestClick = (indexName) => {
  ...
  return (
    <div className={`container my-5 ${darkMode ? 'bg-dark text-light rounded p-3' : ''}`}>
      <h3 className={`mb-4 text-center fw-bold ${darkMode ? 'text-info' : 'text-primary'}`}>Market Indices Real-Time Data</h3>
              ...
              <tr>
                ...
                <td>
                  <button
                    ...
                    disabled={selectedIndex === nifty.index}
                    onClick={() => handleInvestClick(nifty.index)}
                  >
                    {selectedIndex === nifty.index ? 'Selected' : 'Invest'}
                  </button>
                </td>
              </tr>
            )}
            {sensex && (
              <tr>
                ...
                <td>
                  <button
                    className="btn btn-primary"
                    disabled={selectedIndex === sensex.index}
                    onClick={() => handleInvestClick(sensex.index)}
                  >
                    {selectedIndex === sensex.index ? 'Selected' : 'Invest'}
                  </button>
                </td>
              </tr>
            )}
